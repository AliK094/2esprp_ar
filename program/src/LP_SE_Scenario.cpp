...
// ------------------------------------------------------------------------------------------------------------------------
LP_SE_Scenario::LP_SE_Scenario(const ParameterSetting &parameters, const SolutionFirstEchelon &sol_FE, 
							const ScenarioSolutionSecondEchelon &sol_SE_scenario, int s)
	: params(parameters),
	  sol_FE(sol_FE),
...
void LP_SE_Scenario::configureCplex(IloCplex &cplex, IloEnv &env)
{
...
string LP_SE_Scenario::handleCplexStatus(IloCplex &cplex, IloEnv &env, IloModel &model)
{
	string status;
	double objValue = 0.0;
...
string LP_SE_Scenario::generateFileName(const string &baseDir, const string &extension)
{
	string fileName = baseDir + "LP_SE_Scenario_" + params.problemType + "_Ins" + params.instance + "_S" + std::to_string(params.numScenarios) + "_scenario" + std::to_string(scenario);
...
void LP_SE_Scenario::refineConflict(IloCplex &cplex, IloEnv &env, IloModel &model)
{
