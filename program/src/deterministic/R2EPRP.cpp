...
R2EPRP::R2EPRP(const ParameterSetting &parameters,
...
bool R2EPRP::Solve()
{
	try
	{
		...
		if (save_lpFile)
		{
			...
			if (shortageAllowed)
			{
				lpFileName = directory + "R2EPRP_NW" + std::to_string(params.numWarehouses) +
		...
		if (cplex.getStatus() == IloAlgorithm::Optimal)
		{
			...
			if (save_mpsResultFile)
			{
				...
				if (shortageAllowed)
				{
					solFileName = directory + "R2EPRP_NW" + std::to_string(params.numWarehouses) +
										"_NR" + std::to_string(params.numCustomers) +
										"_KP" + std::to_string(params.numVehicles_Plant) +
										"_KW" + std::to_string(params.numVehicles_Warehouse) +
										"_T" + std::to_string(params.numPeriods) +
			...
			for (int k = 0; k < params.numVehicles_Warehouse; ++k)
			{
				...
				if (!route.empty())
				{
					for (int i = 0; i < params.numCustomers; ++i)
					{
						...
						if (it != route.end() - 1)
						{
							IloConstraint customerVisitConstraint(expr <= Del_UB[i][t]);
							expr.end();
