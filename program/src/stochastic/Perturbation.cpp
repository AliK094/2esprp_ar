...
Perturbation::Perturbation(const int s, const ParameterSetting &parameters, const SolutionFirstEchelon &sol_FE, const ScenarioSolutionSecondEchelon &sol_SE_scenario)
...
bool Perturbation::run()
{
	...
	while (perturbIteration < max_perturb)
	{
		...
		sol_SE_scenario_feasible.clear();
		sol_SE_scenario_feasible = lpse_scenario.getSolutionSE_Scenario();
...
ScenarioSolutionSecondEchelon Perturbation::getSolutionSE_Scenario()
{
	return sol_SE_scenario_feasible;
}

double Perturbation::getObjVal_Scenario()
{
	return objVal_feasible_scenario;
}