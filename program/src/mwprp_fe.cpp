	...
	for (int s = 0; s < params.numScenarios; ++s)
	{
		for (int t = 0; t < params.numPeriods; ++t)
		{
			for (int w = 0; w < params.numWarehouses; ++w)
			{
				...
				// cout << "demand_warehouse[" << w << "][" << t << "][" << s << "] = " << demand_warehouse[w][t][s] << endl;
			}
		}
	}
...
void MWPRP_FE::configureCplex(IloCplex &cplex, IloEnv &env)
{
	...
	parameterManager.setParameters(1, params.MWPRP_FE_OptimalityGap, params.MWPRP_FE_TimeLimit, params.MWPRP_FE_NumThreads, params.MWPRP_FE_MemoryLimit);
...
{
	string fileName = baseDir + "MWPRP_FE_" + params.problemType + "_Ins" + params.instance + "_S" + std::to_string(params.numScenarios);
	fileName += extension;
