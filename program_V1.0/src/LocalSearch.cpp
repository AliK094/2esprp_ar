...
LocalSearch::LocalSearch(const int scenarioId, const ParameterSetting &parameters, const Solution &initialSolution)
	: scenario(scenarioId), params(parameters), sol_temp(initialSolution), sol_feasible(initialSolution), rng(std::random_device{}())
{
	// ----------------------------------------------------------------------------------------------------------
...
{
	cout << "\nCurrent Objective Value : " << sol_temp.totalObjValue << endl;
...
bool LocalSearch::Shift(int v)
{
	...
	for (const int t : periods)
	{
		...
		{
			vector<int> &sourceRoute = sol_feasible.routesWarehouseToCustomer[scenario][sourceWarehouse][t][sourceVehicle];
		...
		for (const int sourceVehicle : sourceVehicles)
		{
			...
			for (const int destVehicle : destVehicles)
			{
				...
				for (const int startPos : eligibleStartPositions)
				{
					...
					{
						// Print old routes
						printRoute(scenario, sourceWarehouse, t, sourceVehicle, sourceRoute, "Original Source Route");
						...
						for (const int customer : segment)
						{
							...
							sol_feasible.customerAssignmentToWarehouse[scenario][t][sourceWarehouse][customer - params.numWarehouses] = 0;
						...
						printRoute(scenario, sourceWarehouse, t, sourceVehicle, sourceRoute, "Updated Source Route");
	...
	for (const int t : periods)
	{
		...
		for (const int sourceVehicle : sourceVehicles)
		{
			...
			for (const int destVehicle : destVehicles)
			{
				...
				{
					// Set previous warehouse assignment to 0
					sol_feasible.customerAssignmentToWarehouse[scenario][t][destWarehouse][customer - params.numWarehouses] = 0;

					// Set new warehouse assignment to 1
					sol_feasible.customerAssignmentToWarehouse[scenario][t][sourceWarehouse][customer - params.numWarehouses] = 1;
				}
...
bool LocalSearch::Insert()
{
	...
	for (const int t : periods)
	{
		...
		for (auto &route : sol_feasible.routesWarehouseToCustomer[scenario][selectedWarehouse][t])
		{
			if (route.empty())
			{
		...
		if (routeToIns != -1 && !sol_feasible.routesWarehouseToCustomer[scenario][selectedWarehouse][t][routeToIns].empty())
		{
			// Insert node into the non-empty route
			sol_feasible.routesWarehouseToCustomer[scenario][selectedWarehouse][t][routeToIns].insert(sol_feasible.routesWarehouseToCustomer[scenario][selectedWarehouse][t][routeToIns].begin() + minInsertionPos, unvisitedNode);
			inserted = true;
		}
		else if (routeToIns != -1 && sol_feasible.routesWarehouseToCustomer[scenario][selectedWarehouse][t][routeToIns].empty())
		{
			// If the route is empty, add warehouse -> node -> warehouse
			sol_feasible.routesWarehouseToCustomer[scenario][selectedWarehouse][t][routeToIns] = {selectedWarehouse, unvisitedNode, selectedWarehouse};
			inserted = true;
		}
		...
		if (assignedWarehouse != selectedWarehouse)
		{
			...
			sol_feasible.customerAssignmentToWarehouse[scenario][t][selectedWarehouse][unvisitedNode - params.numWarehouses] = 1;
		}
		...
		for (auto &route : sol_feasible.routesWarehouseToCustomer[scenario][selectedWarehouse][t])
		{
			printRoute(scenario, selectedWarehouse, t, rtIndex, route, "Updated Route");
			rtIndex++;
		}
...
bool LocalSearch::Remove()
{
	...
	for (auto &route : sol_feasible.routesWarehouseToCustomer[scenario][selectedWarehouse][t])
	{
		printRoute(scenario, selectedWarehouse, t, rtIndex, route, "Old Route");

		rtIndex++;
	}

	// Gather routes that have more than just the warehouse start and end node
	for (int r = 0; r < sol_feasible.routesWarehouseToCustomer[scenario][selectedWarehouse][t].size(); ++r)
	{
		if (sol_feasible.routesWarehouseToCustomer[scenario][selectedWarehouse][t][r].size() > 2)
	...
	auto &selectedRoute = sol_feasible.routesWarehouseToCustomer[scenario][selectedWarehouse][t][routeIndex];
	...
	for (auto &route : sol_feasible.routesWarehouseToCustomer[scenario][selectedWarehouse][t])
	{
		printRoute(scenario, selectedWarehouse, t, rtIndex, route, "New Route");
    ...
    for (int periodToInsert : unvisitedPeriods)
    {
        ...
        if (routeToInsert != -1)
        {
            ...
            for (int w = 0; w < params.numWarehouses; ++w)
            {
                ...
                {
                    sol_feasible.customerAssignmentToWarehouse[scenario][periodToInsert][w][customer - params.numWarehouses] = 0;
                }
            }
            ...
            sol_feasible.customerAssignmentToWarehouse[scenario][periodToInsert][warehouseToInsert][customer - params.numWarehouses] = 1;
        }
    }
...
bool LocalSearch::Remove_Insert()
{
    ...
    for (int t : periods)
    {
        ...
        for (int w = 0; w < params.numWarehouses; ++w)
        {
            if (sol_feasible.customerAssignmentToWarehouse[scenario][t][w][selectedCustomer - params.numWarehouses] == 1)
            {
                for (const auto &route : sol_feasible.routesWarehouseToCustomer[scenario][w][t])
                {
    ...
    for (auto &route : sol_feasible.routesWarehouseToCustomer[scenario][warehouseFrom][fromPeriod])
    {
    ...
    for (auto &route : sol_feasible.routesWarehouseToCustomer[scenario][warehouseToInsert][toPeriod])
    {
        if (route.empty())
        {
    ...
    if (routeToInsert != -1)
    {
        ...
        for (int w = 0; w < params.numWarehouses; ++w)
        {
            if (w != warehouseToInsert)
            {
                sol_feasible.customerAssignmentToWarehouse[scenario][toPeriod][w][selectedCustomer - params.numWarehouses] = 0;
            }
        }

        if (sol_feasible.routesWarehouseToCustomer[scenario][warehouseToInsert][toPeriod][routeToInsert].empty())
        {
            // Insert into an empty route
            sol_feasible.routesWarehouseToCustomer[scenario][warehouseToInsert][toPeriod][routeToInsert] = {warehouseToInsert, selectedCustomer, warehouseToInsert};
        }
        else
        {
            // Insert into a non-empty route
            sol_feasible.routesWarehouseToCustomer[scenario][warehouseToInsert][toPeriod][routeToInsert].insert(
                sol_feasible.routesWarehouseToCustomer[scenario][warehouseToInsert][toPeriod][routeToInsert].begin() + minInsertionPos,
                selectedCustomer);
        }

        // Update the customerAssignmentToWarehouse matrix for the new warehouse
        sol_feasible.customerAssignmentToWarehouse[scenario][toPeriod][warehouseToInsert][selectedCustomer - params.numWarehouses] = 1;
    }
