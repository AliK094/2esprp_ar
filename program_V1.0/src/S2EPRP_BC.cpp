...
S2EPRP_BC::S2EPRP_BC(const ParameterSetting &parameters, const Solution &warmStart)
	: params(parameters),
	  THRESHOLD(1e-2),
	  save_lpFile(true),
...
bool S2EPRP_BC::Solve()
{
	try
	{
		...
		if (save_lpFile)
		{
			...
			string lpFileName = directory + "S2EPRP_BC_NW" + std::to_string(params.numWarehouses) + "_NR" + std::to_string(params.numCustomers) + "_KP" + std::to_string(params.numVehicles_Plant) + "_KW" + std::to_string(params.numVehicles_Warehouse) + "_T" + std::to_string(params.numPeriods) + "_S" + std::to_string(params.numScenarios) + "_Ins" + params.instance.c_str() + ".lp";
		...
		{
			sol.status = "Optimal";
			sol.totalObjValue = cplex.getObjValue();
			cout << "Optimal solution found with objective value: " << std::fixed << std::setprecision(1) << sol.totalObjValue << endl;
			sol.optimalityGap = cplex.getMIPRelativeGap() * 100;
			sol.lowerBound = cplex.getBestObjValue();

			if (save_mpsResultFile)
			{
				string directory = "../cplexFiles/solVal/";
				string solFileName = directory + "S2EPRP_BC_NW" + std::to_string(params.numWarehouses) + "_NR" + std::to_string(params.numCustomers) + "_KP" + std::to_string(params.numVehicles_Plant) + "_KW" + std::to_string(params.numVehicles_Warehouse) + "_T" + std::to_string(params.numPeriods) + "_S" + std::to_string(params.numScenarios) + "_Ins" + params.instance.c_str();
		...
		{
			sol.status = "Incumbent";
			sol.totalObjValue = cplex.getObjValue();
			cout << "Incumbent solution found with objective value: " << std::fixed << std::setprecision(1) << sol.totalObjValue << endl;
			sol.optimalityGap = cplex.getMIPRelativeGap() * 100;
			cout << "Optimality gap: " << sol.optimalityGap << "%" << endl;
			sol.lowerBound = cplex.getBestObjValue();
			cout << "Lower bound: " << sol.lowerBound << endl;
		...
		else
		{
			...
			cout << "Solver terminated with status: " << sol.status << endl;
		}

		auto currentTime = std::chrono::high_resolution_clock::now();
		sol.totalCPUTime = std::chrono::duration_cast<std::chrono::duration<double>>(currentTime - startTime).count();
		cout << "Timer (seconds): " << std::fixed << std::setprecision(4) << sol.totalCPUTime << endl;

		if (sol.status == "Optimal" || sol.status == "Incumbent")
		{
		...
		// 	params.saveResultSummaryExact(solution, algorithm, sol.objValue, sol.lowerBound, sol.optimalityGap, sol.totalCPUTime, feasCheck);
		// }
...
void S2EPRP_BC::RetrieveSolutions(IloCplex &cplex)
{
	...
	sol.customerUnmetDemand.assign(params.numCustomers, vector<vector<double>>(params.numPeriods, vector<double>(params.numScenarios, 0.0)));
	...
	sol.deliveryQuantityToCustomer.assign(params.numCustomers, vector<vector<double>>(params.numPeriods, vector<double>(params.numScenarios, 0.0)));
	sol.routesWarehouseToCustomer.assign(params.numScenarios, vector<vector<vector<vector<int>>>>(params.numWarehouses, vector<vector<vector<int>>>(params.numPeriods, vector<vector<int>>(params.numVehicles_Warehouse, vector<int>()))));
	sol.customerAssignmentToWarehouse.assign(params.numScenarios, vector<vector<vector<int>>>(params.numPeriods, vector<vector<int>>(params.numWarehouses, vector<int>(params.numCustomers, 0))));
	...
	for (int t = 0; t < params.numPeriods; ++t)
	{
		...
		for (int i = 0; i < params.numCustomers; ++i)
		{
			for (int s = 0; s < params.numScenarios; ++s)
			{
				sol.customerInventory[i][t][s] = cplex.getValue(I_customer[i][t][s]);
				sol.customerUnmetDemand[i][t][s] = cplex.getValue(b_customer[i][t][s]);
				for (int k = 0; k < params.numVehicles_SecondEchelon; ++k)
				{
					sol.deliveryQuantityToCustomer[i][t][s] += cplex.getValue(w_customer[i][k][t][s]);
				}
			}
		}
			...
			for (int w = 0; w < params.numWarehouses; ++w)
			{
				...
				for (int k : params.set_WarehouseVehicles[w])
				{
					...
					if (visitedNodes_SecondEchelon[w][k][t][s] == 1)
					{
						...
						sol.routesWarehouseToCustomer[s][w][t][vehInd] = route;
						vehInd++;
					}
				}
			}
				...
				for (int i = 0; i < params.numCustomers; ++i)
				{
					for (int k : params.set_WarehouseVehicles[w])
					{
						if (visitedNodes_SecondEchelon[i + params.numWarehouses][k][t][s] == 1)
						{
							...
							sol.customerAssignmentToWarehouse[s][t][w][i] = 1;
							break;
						}
					}
				}
...
void S2EPRP_BC::CalculateCostsForEachPart()
{
	for (int t = 0; t < params.numPeriods; ++t)
	{
		...
		for (int s = 0; s < params.numScenarios; ++s)
		{
			...
			for (int i = 0; i < params.numCustomers; ++i)
			{
				sol.holdingCostCustomer_Avg += params.probability[s] * params.unitHoldingCost_Customer[i] * sol.customerInventory[i][t][s];
				sol.costOfUnmetDemand_Avg += params.probability[s] * params.unmetDemandPenalty[i] * sol.customerUnmetDemand[i][t][s];
			}
		}
	}
	...
	for (int s = 0; s < params.numScenarios; ++s)
	{
		for (int w = 0; w < params.numWarehouses; ++w)
		{
			for (int t = 0; t < params.numPeriods; ++t)
			{
				...
				{
					int previousNode = w;
					for (int j = 1; j < sol.routesWarehouseToCustomer[s][w][t][k].size(); ++j)
					{
						int currentNode = sol.routesWarehouseToCustomer[s][w][t][k][j];

						sol.transportationCostWarehouseToCustomer_Avg += params.probability[s] * params.transportationCost_SecondEchelon[previousNode][currentNode];
	...
	double objValue_FE = sol.setupCost + sol.productionCost + sol.holdingCostPlant + sol.transportationCostPlantToWarehouse;
	cout << "setupCost (BC) = " << sol.setupCost << endl;
	cout << "productionCost (BC) = " << sol.productionCost << endl;
	cout << "holdingCostPlant (BC) = " << sol.holdingCostPlant << endl;
	cout << "transportationCostPlantToWarehouse (BC) = " << sol.transportationCostPlantToWarehouse << endl;
	cout << "objValue_FE (BC) = " << objValue_FE << endl;

	cout << "Avg holding cost warehouse (BC) : " << sol.holdingCostWarehouse_Avg << endl;
	cout << "Avg holding cost customer (BC) : " << sol.holdingCostCustomer_Avg << endl;
	cout << "Avg cost of unmet demand (BC) : " << sol.costOfUnmetDemand_Avg << endl;
	cout << "Avg transportation cost warehouse to customer (BC) : " << sol.transportationCostWarehouseToCustomer_Avg << endl;
	double objValue_SE = sol.holdingCostWarehouse_Avg + sol.holdingCostCustomer_Avg + sol.costOfUnmetDemand_Avg + sol.transportationCostWarehouseToCustomer_Avg;

	sol.totalObjValue = objValue_FE + objValue_SE;

	cout << "Objective value FE : " << objValue_FE << endl;
	cout << "Objective value (ILS) SE : " << objValue_SE << endl;
	cout << "Objective value (ILS) Total : " << sol.totalObjValue << endl;
}
...
void S2EPRP_BC::DisplayCustomerInventoryVars()
{
	for (int s = 0; s < params.numScenarios; ++s)
	{
		for (int t = 0; t < params.numPeriods; ++t)
		{
			for (int i = 0; i < params.numCustomers; ++i)
			{
				if (sol.customerInventory[i][t][s] > THRESHOLD)
				{
					cout << "I_customer[" << i + params.numWarehouses + 1 << "][" << t + 1 << "][" << s + 1 << "] = " << sol.customerInventory[i][t][s] << endl;
				}
			}
		}
	}
...
void S2EPRP_BC::DisplayCustomerUnmetDemandVars()
{
	for (int s = 0; s < params.numScenarios; ++s)
	{
		for (int t = 0; t < params.numPeriods; ++t)
		{
			for (int i = 0; i < params.numCustomers; ++i)
			{
				if (sol.customerUnmetDemand[i][t][s] > THRESHOLD)
				{
					cout << "b_customer[" << i + params.numWarehouses + 1 << "][" << t + 1 << "][" << s + 1 << "] = " << sol.customerUnmetDemand[i][t][s] << endl;
				}
			}
		}
	}
...
void S2EPRP_BC::DisplayDeliveryQuantityToCustomersVars()
{
	for (int s = 0; s < params.numScenarios; ++s)
	{
		for (int t = 0; t < params.numPeriods; ++t)
		{
			for (int i = 0; i < params.numCustomers; ++i)
			{
				if (sol.deliveryQuantityToCustomer[i][t][s] > THRESHOLD)
				{
					cout << "deliveryQuantityToCustomer[" << i + params.numWarehouses + 1 << "][" << t + 1 << "][" << s + 1 << "] = " << sol.deliveryQuantityToCustomer[i][t][s] << endl;
				}
			}
		}
	}
	...
	for (int s = 0; s < params.numScenarios; ++s)
	{
		for (int w = 0; w < params.numWarehouses; ++w)
		{
			for (int t = 0; t < params.numPeriods; ++t)
			{
				for (int k = 0; k < params.numVehicles_Warehouse; ++k)
				{
					if (!sol.routesWarehouseToCustomer[s][w][t][k].empty())
					{
						cout << "route[" << s + 1 << "][" << w + 1 << "][" << t + 1 << "][" << k + 1 << "] : [";
						for (auto it = sol.routesWarehouseToCustomer[s][w][t][k].begin(); it != sol.routesWarehouseToCustomer[s][w][t][k].end(); ++it)
						{
							if (it != sol.routesWarehouseToCustomer[s][w][t][k].begin())
							{
								cout << " -> ";
							}
							cout << *it;
...
void S2EPRP_BC::DefineWarmStartSolution(IloEnv &env, IloCplex &cplex)
{
	...
	for (int t = 0; t < params.numPeriods; ++t)
	{
		for (const auto &route : warmStart.routesPlantToWarehouse[t])
		{
			auto it = std::find(optimalRoutes_FirstEchelon.begin(), optimalRoutes_FirstEchelon.end(), route);
			// cout << "Route (Plant To Warehouse) [" << t + 1 << "] : [" ;
	...
	// 			for (int k = 0; k < params.numVehicles_Warehouse; ++k)
	// 			{
	// 				if (!warmStart.routesWarehouseToCustomer[s][w][t][k].empty())
	// 				{
	// 					cout << "route[" << s + 1 << "][" << w + 1 << "][" << t + 1 << "][" << k + 1 << "] : [";
	// 					for (auto it = warmStart.routesWarehouseToCustomer[s][w][t][k].begin(); it != warmStart.routesWarehouseToCustomer[s][w][t][k].end(); ++it)
	// 					{
	// 						if (it != warmStart.routesWarehouseToCustomer[s][w][t][k].begin())
	// 						{
	// 							cout << " -> ";
	// 						}
	// 						cout << *it;
		...
		for (int w = 0; w < params.numWarehouses; ++w)
		{
			for (int t = 0; t < params.numPeriods; ++t)
			{
				for (int k = 0; k < params.numVehicles_Warehouse; k++)
				{
					...
					vector<int> &route = warmStart.routesWarehouseToCustomer[s][w][t][k];
					if (!route.empty())
					{
						// cout << "route[" << s + 1 << "][" << w + 1 << "][" << t + 1 << "][" << vehicleIndex + 1 << "] : [";
						// for (auto it = route.begin(); it != route.end(); ++it)
						// {
						// 	if (it != route.begin())
						// 	{
						...
						startVal.add(1);
						// cout << "z[" << w << "][" << vehicleIndex + 1 << "][" << t + 1 << "][" << s + 1 << "] = 1" << endl;
						...
						for (int i = 0; i < params.numCustomers; ++i)
						{
							...
							if (it != route.end())
							{
								...
								startVal.add(1);
								// cout << "z[" << customerIndex << "][" << vehicleIndex + 1 << "][" << t + 1 << "][" << s + 1 << "] = 1" << endl;
							}
							else
							{
								startVar.add(z[customerIndex][vehicleIndex][t][s]);
	...
	// 			for (int e = 0; e < params.numEdges_SecondEchelon; ++e)
	// 			{
	// 				if (x_warmStart[e][k][t][s] > 0)
	// 				{
	// 					cout << "x[" << params.index_i_SecondEchelon[e] << "][" << params.index_j_SecondEchelon[e] << "][" << k + 1 << "][" << t + 1 << "][" << s + 1 << "] = " << x_warmStart[e][k][t][s] << endl;
	// 				}
	// 			}
	// 		}
	// 	}
