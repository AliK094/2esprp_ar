...
bool Algorithms::solve_S2EPRP_HILS(Solution &solFinal, const Solution &initSol)
{
	...
	// cout << "Before Optimizing Unmet Demand and Routes:" << endl;

	// cout << "Customers Assigned To Warehouses:" << endl;
	// for (int s = 0; s < params.numScenarios; ++s)
	// {
	// 	for (int t = 0; t < params.numPeriods; ++t)
	// 	{
	// 		for (int w = 0; w < params.numWarehouses; ++w)
	// 		{
	// 			cout << "s = " << s + 1 << ", t = " << t + 1 << ", w = " << w + 1 << ": ";
	// 			for (int i = 0; i < params.numCustomers; ++i)
	// 			{
	// 				if (solFinal_temp.customerAssignmentToWarehouse[s][t][w][i] > 1e-4)
	// 				{
	...
	// cout << "Customers Unmet Demand:" << endl;
	// for (int s = 0; s < params.numScenarios; ++s)
	// {
	// 	for (int t = 0; t < params.numPeriods; ++t)
	// 	{
	// 		for (int i = 0; i < params.numCustomers; ++i)
	// 		{
	// 			if (solFinal_temp.customerUnmetDemand[i][t][s] > 1e-4)
	// 			{
	// 				cout << "s = " << s + 1 << ", t = " << t + 1 << ", i = " << i + params.numWarehouses << ": " << solFinal_temp.customerUnmetDemand[i][t][s] << endl;
	// 			}
	// 		}
	// 	}
	// }
	...
	// 			for (int k = 0; k < params.numVehicles_Warehouse; ++k)
	// 			{
	// 				vector<int> route = solFinal_temp.routesWarehouseToCustomer[s][w][t][k];
	// 				if (!route.empty())
	// 				{
	// 					cout << "s = " << s + 1 << ", w = " << w + 1 << ", t = " << t + 1 << ", k = " << k + 1 << ": ";
	// 					for (size_t i = 0; i < route.size(); ++i)
	...
	// optimizeUnmetDemandAndRoutes(solFinal_temp);

	// cout << "After Optimizing Unmet Demand and Routes:" << endl;
	// cout << "Customers Assigned To Warehouses:" << endl;
	// for (int s = 0; s < params.numScenarios; ++s)
	// {
	// 	for (int t = 0; t < params.numPeriods; ++t)
	...
	// // Solve the restricted problem and finalize the solution
	// if (!solveRestrictedProblemAndFinalize(solFinal_temp))
	// {
	// 	return EXIT_FAILURE;
	// }
